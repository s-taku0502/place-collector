...
1. 詳細（`/place/[id]/detail`）で基本情報と「行く前メモ」を確認できます。
1. 実際に訪問したら詳細画面の「行った！」を押します。ステータスが「行った（また行きたい）」に更新され、フィードバック画面（`/place/[id]/detail/feedback`）へ遷移します。
1. フィードバック画面で、☆5評価／また行きたいか／メモを入力して保存します。フィードバックは複数回追加できます（訪問のたびに残せます）。
1. 詳細画面の下部に、最新順で過去のフィードバックが表示されます。
